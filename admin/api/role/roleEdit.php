...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Kiểm tra nếu không có ID trong dữ liệu JSON
    if (!isset($inputData['roleId']) || $inputData['roleId'] == "") {
        http_response_code(400);
    ...
    // sửa quyền vào cơ sở dữ liệu
    $result = $database->updateRole($inputData['roleId'], $inputData['roleName'], $inputData['permissions']);
    
    // Kiểm tra kết quả thêm quyền
    if ($result) {
        http_response_code(200);
        echo json_encode(["status" => "success", "message" => "Sửa quyền thành công"]);
    } else {
        http_response_code(500);
        echo json_encode(["status" => "error", "message" => "Lỗi khi sửa quyền"]);
    }
